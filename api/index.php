...
case'planessave':
	...
	$ap=$_POST["prec"];
	$ci=$_POST["des"];
	$BDD->query("insert into planes (nomp,prec,des) values ('$nomo','$prec','$des') ");

	$M=$BDD->query("select max(nump) as nump, nomp from planes");
	foreach($M as $V)
	{	$nump=$V["nump"];
	}
	$data=array("resp"=>true,"msg"=>"se guardo", "nump"=>$nump,"nomp"=>$nomp);
