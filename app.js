	...
	<main>
		<section class='axsec'>
			<nav  class='axmenmax' id='ML'>
				...
				if(tipo=='Admin')
					{ TK+=`
						<a onclick="planesfrm()">Registrar plan</a>
						<a onclick="userfrm()">Registrar persona</a>
						<a onclick="excellist1()">Leer Excel</a>`; }
				TK+=`
...
{	let tipo=axtokenget('aud')[0];
	...
	{	TK+=`
		<section  id='PLANES${data.list[P].num}'  class='axtitlemin'>
			<h2 class='axa-l'>${data.list[P].nom} (${data.list[P].prec} Bs)</h2>
			<a  class='axico135'  onclick="planesdel(${data.list[P].num})"></a>}
		</section>`;
	}
	TK+=`
	<form id='F8' class='axfrmmin'>
		<center>Nuevo plan</center>
		<input nmae='nomp' type='text' placeholder='Nombre del plan'>
		<input nmae='prec' type='number placeholder='Precio'> 
	...
	{ e.preventDefault();
		...
		{axmsgok('Datos guardados');	
			...
			<section id='PLANES${data.nump}' class='axtitlemin'>
				...
				<a class='axico35' onclick="planesdel(${data.nump}"></a>
			</section>`;
...
{	let data= await axdataget(`api/excellist1`);
	console.log(data);
	let TK=`
	<center class='axtitle'>Lista del Excel</center>`;
	if(data.resp)
	{	for(P=0; P<data.list.length; P++)
		{	TK+=`
